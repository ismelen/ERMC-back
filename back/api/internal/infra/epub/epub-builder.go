...
func (b *EpubBuilder) Start(name string, outDir string) manga.BookBuilder {
	pageSide := PAGE_RIGHT
...
func (b *EpubBuilder) AddPage(page *manga.Page, fstPage bool) manga.BookBuilder {
	b.mu.Lock()
	defer b.mu.Unlock()
	...
	for _, part := range page.Parts {
		...
		switch part.Split {
		case manga.SplitNone, manga.SplitRotated:
			b.pageSide = strutil.Toggle(b.pageSide, PAGE_RIGHT, PAGE_LEFT)
		case manga.SplitToLeft:
			b.pageSide = PAGE_LEFT
		case manga.SplitToRight:
...
func (b *EpubBuilder) addHTML(part *manga.PagePart, CSSBgStyle string) error {
